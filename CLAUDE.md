...
- `amount: 0` は「価格未確認」として表示される（プレースホルダーに使える）。エントリに
  `price_label` があればその文字列を表示（例:「プライム会費に含まれる」）
- サービス/プランの任意フィールド `ended: "YYYY-MM-DD"` = 提供終了（グレー表示・終了バッジ・
  シミュレーター対象外・ニュースに「終了」掲載。**削除せず履歴を残すこと**）。
  プランの `closed: true` = 新規受付終了バッジ（契約継続中の人がいるためシミュレーターでは選択可）
...
ユーザーが「デスク方式で」と指示したら：**デスク（Fable等の上位モデル＝メインループ）が仕様確定・
出典確認・検収を行い、実装はSonnet等の軽量モデルのエージェントに委任**する。
- デスク: 変更仕様を正確な置換文字列レベルまで確定し、エージェントに完全な指示を出す。
  完了後にプレビューで検収し、合格ならコミット・push
- 実装エージェント: CLAUDE.mdを読み、UTF-8 BOM等の規約に従って実装。コミットはしない
- 調査系はWorkflow並列（Research=Sonnet/Verify=上位モデル）も同じ思想
...
- サービス追加の将来候補（今回は数を抑える方針で見送り）: 読書系カテゴリ（Kindle Unlimited・
  Audible・dマガジン）、クリエイティブ拡充（CLIP STUDIO PAINT・Canva Pro）、GitHub Copilot
- 合計金額シミュレーターはプロトタイプ実装済み（チェック→下部バーに合計、自由入力枠あり）。
  公開時に載せるかは別途判断。拡張候補: 値上げ影響額（1年前比）、選択状態のURL共有
- 改定ニュース欄の見せ方を再検討（現状やや目立ちすぎ。折りたたみ・件数減・トーン調整などの誘導方法は今後決める）
- 将来的に技術担当へ引き継ぐ予定。暫定運用に固有の仕組み（GitHub API化・トークン管理など）は
  追加しない方針。技術仕様書はREADME.md、運用手順はdocs/MANUAL.mdに整備済み（変更時は両方を更新）
