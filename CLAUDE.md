...
- **価格改定の反映**: 旧エントリのvalid_toを改定前日で閉じる → 新エントリ（valid_from=改定日、
  source=出典URL、note=改定内容）を追加。直近12カ月以内の改定は一覧に値上げ/値下げバッジが自動表示される
...
- 検討中の機能: 合計金額シミュレーター、今月の改定ニュース欄、サービス追加（読書系カテゴリ等）、
  edit.htmlの価格改定専用フロー、スマホ編集対応（edit.htmlのGitHub API化）
