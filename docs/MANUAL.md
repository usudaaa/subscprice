...
### 5.5 サービス終了・名称変更などライフサイクルの記録

**サービスが終了したら** — 基本情報の「サービス終了日」に日付を入力して保存。
- 一覧がグレー表示＋「提供終了」バッジになり、改定ニュースにも「終了」として自動掲載
- 合計シミュレーターの対象外になります
- **削除はしないこと**。価格の歴史こそがこのサイトの資産です（削除すると履歴ごと消えます）

**プランが終了したら** — 対象プランの「提供終了日」に日付を入力。プラン単位で同様の表示になります。

**新規受付だけ終了したら**（提供は継続） — プランの「新規受付」を「新規受付終了」に変更。「新規受付終了」バッジが付きます（例: Adobeフォトプラン 20GB）。

**名称が変わったら** — サービス名/プラン名を書き換えるだけでOK。ただし2つの作法を守ること：
1. **IDは変えない**（記事からの直リンク`#id`や読者のシミュレーター選択が壊れるため）
2. **旧名を概要説明に残す**（例:「旧Yahoo!プレミアム」）。検索は概要説明にもヒットするので、旧名で探す読者にも見つかります
...
7. サービス・プランは**削除せず「終了日」で記録**する（履歴の保全）。IDは改名時も変更しない
...
- **データのバックアップ**: 編集ツールの「全データをコピー／エクスポート」からJSON全文を取得できます。また、GitHubへのpushがそのまま履歴付きバックアップになっています
- **編集部での再確認リスト**: 収録時点確認のままの価格12件を `docs/EDITORIAL_CHECK.md` にまとめてあります
- 技術仕様の詳細はリポジトリ直下の `README.md`（技術担当向け）、AI向け開発ルールは `CLAUDE.md` を参照
